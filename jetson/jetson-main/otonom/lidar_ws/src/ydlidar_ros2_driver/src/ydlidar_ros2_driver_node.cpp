...
int main(int argc, char *argv[]) {
  ...
  /// sample rate
  optval = 9;
  node->declare_parameter("sample_rate", 4);
  ...
  /// Intenstiy bit count
  optval = 8;
  node->declare_parameter("intensity_bit", 10);
  ...
  //////////////////////float property/////////////////
  /// unit: °
  float f_optvalue = 110.0f;
  node->declare_parameter("angle_max", 110.0);
  node->get_parameter("angle_max", f_optvalue);
  laser.setlidaropt(LidarPropMaxAngle, &f_optvalue, sizeof(float));
  f_optvalue = -110.0f;
  node->declare_parameter("angle_min", -110.0);
  node->get_parameter("angle_min", f_optvalue);
  laser.setlidaropt(LidarPropMinAngle, &f_optvalue, sizeof(float));
  /// unit: m
  f_optvalue = 64.f;
  node->declare_parameter("range_max", 10.0);
  ...
  while (ret && rclcpp::ok()) {
    ...
    if (laser.doProcessSimple(scan)) {
      ...
      int size = (scan.config.max_angle - scan.config.min_angle)/ scan.config.angle_increment +2;
