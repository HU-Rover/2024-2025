    ...
    def timer_callback(self):
        ...
        if ids is not None:
            ...
            for i, corner in enumerate(corners):
                ...
                odom_position = tf.translation_from_matrix(transform_matrix)
                odom_quat = tf.quaternion_from_matrix(transform_matrix)
                ...
                aruco_link.header.frame_id = 'odom'
                aruco_link.child_frame_id = 'aruco'
                aruco_link.transform.translation.x = odom_position[0]
                aruco_link.transform.translation.y = odom_position[1]
                aruco_link.transform.translation.z = odom_position[2]
                aruco_link.transform.rotation.x = odom_quat[0]
                aruco_link.transform.rotation.y = odom_quat[1]
                aruco_link.transform.rotation.z = odom_quat[2]
                aruco_link.transform.rotation.w = odom_quat[3]
                ...
                rclpy.time.Time()
                    ...
                    self.get_logger().info(f"Aruco position in 'map': x={translation.x:.2f}, y={translation.y:.2f}, z={translation.z:.2f}")
                    self.get_logger().info(f"Orientation (quaternion): x={rotation.x:.2f}, y={rotation.y:.2f}, z={rotation.z:.2f}, w={rotation.w:.2f}")
                ...
                cv2.putText(self.image, f"Pos: {np.round(odom_position, 2)}", (10, 60 + 30 * i), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
